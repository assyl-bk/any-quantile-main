...
class Coverage(Metric):
    ...
    def update(self, preds: torch.Tensor, target: torch.Tensor, q: torch.Tensor) -> None:
        ...
        # Build masks for the two extreme quantiles (low/high). Use to(dtype) for arithmetic.
        mask_high = (q == self.level_high)
        mask_low = (q == self.level_low)

        # Count occurrences per sample (avoid division by zero)
        num_high = mask_high.sum(dim=-1, keepdims=True)
        num_low = mask_low.sum(dim=-1, keepdims=True)

        # Sum predictions corresponding to those quantiles and divide safely
        preds_high_num = (preds * mask_high.to(preds.dtype)).sum(dim=-1, keepdims=True)
        preds_low_num = (preds * mask_low.to(preds.dtype)).sum(dim=-1, keepdims=True)

        preds_high = _divide_no_nan(preds_high_num, num_high.to(preds_high_num.dtype))
        preds_low = _divide_no_nan(preds_low_num, num_low.to(preds_low_num.dtype))

        # boolean mask where target lies between low (inclusive) and high (exclusive)
        inside = ((target < preds_high) & (target >= preds_low)).to(self.numerator.dtype)

        self.numerator += inside.sum()
        self.denominator += float(torch.numel(target))

    def compute(self):
        return _divide_no_nan(self.numerator, self.denominator)
...
class CRPS(Metric):
    ...
    def compute(self):
        return 2 * _divide_no_nan(self.numerator, self.denominator)
